...
namespace Talent.Services.Profile.Controllers
{
    ...
    public class ProfileController : Controller
    {
        ...
        //MOS25072021 added for standard task Module 1
        [HttpPost("createUpdateLanguage")]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "talent")]
        public async Task<IActionResult> CreateUpdateLanguage([FromBody] UserLanguage language)
        {
            ...
            try
            {
                ...
                if (language.Id == "")
                {
                    ...
                    string newLanguageID = await _profileService.AddLanguage(language);
                    message = "Language added successfully with id " + newLanguageID;
                }
                else
                {
                    string userId = _profileService.GetLanguageByIDAsync(language.Id).Result.UserId;
                    ...
                    {
                        return Json(new { Success = false, Message = "You are not authorised to update this language record" });
                    }
                }
            ...
            {
                return Json(new { Success = false, Message = "Error while adding user Language" });
            }
        }
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "talent")]
        public async Task<ActionResult> DeleteLanguage([FromBody] string id)   //changed parameter from AddLanguageViewModel language 
        {
            try
            {
                //check if userid is currentuser
                string userId = _profileService.GetLanguageByIDAsync(id).Result.UserId;

                if (userId == _userAppContext.CurrentUserId)
                {
                    await _profileService.DeleteLanguageByIdAsync(id);
                    return Json(new { Success = true, Message = "Language deleted successfully" });
                }
                else
                    return Json(new { Success = false, Message = "You are not authorised to delete this language" });
            }
            catch (Exception e)
            {
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "talent")]
        public async Task<IActionResult> DeleteSkill([FromBody] string id) //changed parameter from AddSkillViewModel skill
        {
            //MOS25072021 added for standard task Module 1
            try
            {
                //check if userid is currentuser
                string userId = _userSkillRepository.GetByIdAsync(id).Result.UserId;

                if (userId == _userAppContext.CurrentUserId)
                {
                    await _profileService.DeleteSkill(id);
                    return Json(new { Success = true, Message = "Skill successfully deleted" });
                }
                else
                    return Json(new { Success = false, Message = "You are not authorised to delete this skill" });
            }
            catch (Exception e)
            {
        ...
        //MOS25072021 added for standard task Module 1
        [HttpPost("createUpdateSkill")]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "talent")]
        public async Task<IActionResult> CreateUpdateSkill([FromBody] UserSkill skill)
        {
            ...
            try
            {
                ...
                if (skill.Id == "")
                {
                    ...
                    message = "Language added successfully with id " + newSkillID;
                }
                else
                {
                    string userId = _userSkillRepository.GetByIdAsync(skill.Id).Result.UserId;
                    ...
                    {
                        return Json(new { Success = false, Message = "You are not authorised to update this skill record" });
                    }
                }
                return Json(new { Success = true, Message = message });
            }
        ...
        //MOS25072021 added for standard task Module 1
        [HttpPost("createUpdateExperience")]
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "talent")]
        public async Task<IActionResult> CreateUpdateExperience([FromBody] UserExperience experience)
        {
            ...
            try
            {
                ...
                if (experience.Id == "")
                {
                    ...
                    string newExperienceID = await _profileService.AddExperience(experience);
                    message = "Experience added successfully with id " + newExperienceID;
                }
                else
                {
                    string userId = _userExperienceRepository.GetByIdAsync(experience.Id).Result.UserId;
                    ...
                    {
                        return Json(new { Success = false, Message = "You are not authorised to update this experience record" });
                    }
                }
                return Json(new { Success = true, Message = message });
            }
            ...
            {
                return Json(new { Success = false, Message = "Error while adding user experience" });
            }
            //Your code here;
            //throw new NotImplementedException();
        }
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "talent")]
        public async Task<ActionResult> DeleteExperience([FromBody] string id)
        {
            try
            {
                //check if userid is currentuser
                string userId = _userExperienceRepository.GetByIdAsync(id).Result.UserId;

                if (userId == _userAppContext.CurrentUserId)
                {
                    await _profileService.DeleteExperience(id);
                    return Json(new { Success = true, Message = "Experience successfully deleted" });
                }
                else
                    return Json(new { Success = false, Message = "You are not authorised to delete this Experience" });
            }
            catch (Exception e)
            {
                return Json(new { Success = false, Message = "Error while deleting Experience" });
            }
        }
